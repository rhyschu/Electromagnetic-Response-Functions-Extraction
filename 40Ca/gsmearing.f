...
!--------------------------------------------------------------------
      ...
      real*8 pf,pf2,kf,qv,es,dw2des,fyuse,fyuse2,epr,kbar2,fcor
...
c      C1 = 0.0
      q20 = xvalc(16)
      C2 = (1.0+C1*q20/(q2+q20))**xvalc(39)

      a0 = 1.0+C1
      
      
      if(A.EQ.12.0) exmin = 0.0165
      If(A.EQ.27.0) exmin = 0.0085
      If(A.EQ.40.0) exmin = 0.0085
      ...
      !!! energy shift !!!
        Es = 0.0085
      !!! fermi momentum  !!!
        kf = xvalc(25)+0.0015*(sqrt(A/12.0)-1.0)
        qvt = qv
        if(qv.GE.qvmax) qvt = qvmax
        Es = xvalc(26)
        dES = xvalc(27)
        Es = Es-dES*(qvmax-qvt)**1.5 !!!  test  !!!
      endif
...
c           write(6,2000) w2,q2,ism,xxp(ism),fyuse, wsqp, fytot
        ...
        t1 = 1.0+xvalc(37)*(A/100.0)**xvalc(38)
        t1 = xvalc(37)+xvalc(38)*(sqrt(A/12.0)-1.0)
        t1 = sqrt(t1) 
        ...
        do j=1,1
           ...
           IF(w2t.GT. 1.159) THEN
             xt = q2/(q2+W2t-mp2)
             xp = 1.0D0+(w2t-mp2)/(q2+xvalc(36)*t1) !!! fixed on 11/02/23, mp -> mp2
...
CCC   Next is medium modification factor  CCC
             ...
             emcfac = (1.0-xvalc(11))*
     &           (1.0-(1.0-xp**0.75)**(xvalc(12))+C1)
             emcfac = xvalc(15)+
     &              emcfac/(1.+xvalc(13)*xp*xp)
             
            emcfacL = xvalc(17)*(1.0D0+xvalc(18)*xp*xp)*
     &       (1.+xvalc(19)*xp*xp*xp*xp)*exp(-1.0*xvalc(20)*xp)
...
c      write(6,*) "In Gsmearing: ", w2,q2,f1
