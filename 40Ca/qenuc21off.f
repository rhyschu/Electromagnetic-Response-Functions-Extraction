...
c outputs: F1, F2 (real*8) are structure functions per nucleus
      ...
      REAL*8 F1ff,F2ff,GMoff,GEoff,moff,xvalc(100),B,xm,m,BL,cof
      REAL*8 g1,g2,g3,d1
...
!  Get energy shift and fermi momementum from fit  !      
      ...
      if(IA.GT.2) then
         Esmin = 0.0085           !!! Minimum Es given by removal energy
         if(IA.EQ.12) Esmin = 0.0165
c         if(IA.EQ.27) Esmin = 0.0107
         if(IA.EQ.27) Esmin = 0.010       
         if(IA.EQ.40) Esmin = 0.010
c         kf = xvalc(23)
         kf = xvalc(23)+0.0085*(sqrt(A/12.0)-1.0)
         Es = Esmin + xvalc(24)  !!! Maximum Es
         if(qv.LT.1.5) Es = Es-xvalc(24)*(1.0-qv/1.5)**0.1  !!!  test  !!!        
      endif 
      nup = nu-Es
