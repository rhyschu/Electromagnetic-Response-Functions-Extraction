...
def QE_shift(df, nu_shift):
    ...
    for _, row in df.iterrows():
        updated_nu = (row["nu"] + nu_shift).round(5)
        if updated_nu in nu_to_RTQE:
            RTQE_prime = nu_to_RTQE[updated_nu]
            RLQE_prime = nu_to_RLQE[updated_nu]
            RTNS_prime = nu_to_RTNS[updated_nu] * nuc_factor
            RLNS_prime = nu_to_RLNS[updated_nu] * nuc_factor
            row["RTTOT"] = row["RTTOT"] - row["RTQE"] + RTQE_prime - row["RTNS"] + RTNS_prime
            row["RTQE"] = RTQE_prime
            row["RTNS"] = RTNS_prime
            row["RLTOT"] = row["RLTOT"] - row["RLQE"] + RLQE_prime - row["RLNS"] + RLNS_prime
            row["RLQE"] = RLQE_prime
            row["RLNS"] = RLNS_prime
