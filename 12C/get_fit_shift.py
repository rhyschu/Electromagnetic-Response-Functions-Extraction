...
if __name__ == "__main__":
    ...
    with open(data_fit, "w") as response_file:
        response_file.write("Data:Z,A,E0,ThetaDeg,nu,cross,error,dataSet,Fit:ep,theta,nu,Ex,w2,q2,sigtot,sigqe,sigie,sigmec,nuccstot,signonuc,sigtot_shifted,qe_shifted,ns_shifted,ratio\n")
    for index, row in df.iterrows():
        ...
        sigtot_shifted = values[6] - values[7] + values_shifted[7] - values[10] + values_shifted[10] * elem_dict[elem]["NS_factor"]
        ...
        if sigtot_shifted == 0 or float(values[3]) < elem_dict[elem]["ex_cut_lower"] or float(values[3]) > elem_dict[elem]["ex_cut_upper"]:
            continue
